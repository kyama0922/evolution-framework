...
FrameworkResult::_RESULT EVOLUTION::FUNCTION::CreatePlate(FRAMEWORK::GAMEOBJECT::IMesh** pp_mesh, f32 w, u32 h, u32 d, EVOLUTION::GRAPHIC::IGraphicFactory* graphic_factory, EVOLUTION::GRAPHIC::IGraphicCommand* command){
    ...
    for (u32 z = 0; z < d + 1; z++)
    {
        ...
        {
            int index = (z * (d + 1)) + x;
            work_vertex[index].position.x = (((float)x * (float)w / (float)d)) - ((float)w / 2.0f);
            work_vertex[index].position.y = -((((float)z * (float)h / (float)d)) - ((float)h / 2.0f)); 
            work_vertex[index].position.z = 0.0f;
    ...
    for (u32 z = 0; z < d; z++)
    {
        for (u32 x = 0; x < d; x++)
        {
            int index = (z * (d + d) * 3) + (6 * x);
            int y_add = (z * (d + 1));
            indexes[index] = y_add + x;
            indexes[index + 1] = y_add + x + 1;
            indexes[index + 2] = y_add + x + d + 2;
            indexes[index + 3] = y_add + x + d + 2;
            indexes[index + 4] = y_add + x + d + 1;
            indexes[index + 5] = y_add + x;
        }
    }
